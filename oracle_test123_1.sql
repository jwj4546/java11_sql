...
insert into member values (mem_seq.nextval, 'ki', '1414', '김기태', '1981-12-25', 'kim@gmail.com');

-- 데이터 검색
select * from member;
select id, name, birth from member;

-- 생일(birth)이 1980년생~1985년생의 회원의(member) id, name 컬럼 검색
...
-- id가 'kim', 'lee', 'cho'가 아닌 회원(member) 정보를 검색
select * from member where id not in ('kim', 'lee', 'cho');

-- 컬럼명이 너무 길거나 수식이나 함수를 적용하여 컬럼을 구성할 경우 컬럼에 대한 alias(별칭)를 붙일 수 있다.
-- 회원(member) 테이블로부터 id, name, 이름 중에서 성씨를 검색
select id, name, substr(name,1,1) as surname from member;
-- java에서 rs.getString("surname"); 처럼 alias로 호출해야함

select * from member;

-- id가 'kim'인 회원의 이메일(email)을 'kim@gmail.com'로 변경
update member set email='kim@gmail.com' where id='kim';

-- id가 'kim'인 회원을 강제 탈퇴하도록 한다.
...
-- 외부 조인1(left outer join)
select a.no, a.name, b.pname from emp a right outer join pos b on a.pcode=b.pcode;

-- 외부 조인2(right outer join)
select a.no, a.name, b.pname from emp a right outer join pos b on a.pcode=b.pcode;
...
select pcode, count(emp.no) as cnt from emp group by pcode;

-- 그룹화하는 항목과 출력되는 그룹 항목이 달라서(오류)
select pos.pname, count(emp.no) as cnt from pos, emp where pos.pcode = emp.pcode group by pos.pcode;

-- 직위별 인원수 join문 -> 그룹화하는 항목 : 직위명(pname)
select pos.pname, count(emp.no) as cnt from pos inner join emp on pos.pcode = emp.pcode group by pos.pname;
