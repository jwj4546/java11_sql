...
-- WorkBench에서 해당 내용을 설정하고 싶다면, SET 명령에 해당 옵션을 설정하면 된다.
...
-- 37) 판매(sales) 테이블의 회원별 구매 건수(cnt)를 출력하되 회원아이디(id)와 cnt를 표시하되 컬럼명은 cnt로 하며, 회원아이디(id)의 오름차순 정렬되어 표시하고, 구매건수(cnt)가 2건 이상인 레코드만 검색되도록 하시오.
select id, count(*) as cnt from sales group by id having cnt>=2 order by id asc;
-- 2건 이상 추출 시 group by에 의한 조건문을 사용할 때 where 대신 having 사용

-- 38) 판매(sales) 테이블의 도서별 판매금액의 합계를 구하여 표시하되, 도서코드(bno), 판매금액합계 로 출력되게 하시오.
select bno, sum(money) as "판매금액합계" from sales group by bno;

select b.bno, a.booktitle, sum(b.money) as "판매금액합계" from book a inner join sales b on a.bookid=b.bno group by bno;
...
select id, bno, max(money) as "최대금액" from sales group by id, bno;
...
-- create sequence sd_seq start with 1 increment;						-- MariaDB와 MySQL은 sequence개념이 사용되지 않는다. 또한 사용하려면 별도로 설치가 필요하다.
...
select distinct a.id, a.name, a.tel from member a inner join sales b on a.id=b.id;

-- 59) 내부조인을 활용하여 구매한 도서의 도서코드(bookid), 도서명(bookname), 총판매수량합계(amount), 총판매금액(money)을 조회하시오 단, 출력 내용은 도서코드(bookid)의 오름차순으로 하시오.
...
-- 연관쿼리
select distinct a.booktitle, a.bookprice, a.author from book a where not exists (select 1 from sales b where a.bookid=b.bno);

-- 교집합
select distinct a.booktitle, a.bookprice, a.author from book a, sales b where a.bookid=b.bno;
...
-- 연관쿼리
select distinct a.id, a.name from member a where not exists(select 1 from sales b where a.id=b.id);

-- 62) 회원(member)와 회원2(member2) 테이블을 교집합하여 중복회원 뷰(cross_mem_view)를 생성하시오. 
create view cross_mem_view as select a.id, a.password, a.name, a.address from member a, member2 b where a.id=b.id; 
select * from cross_mem_view;

-- 63) 도서(book)와 도서2(book2) 테이블을 차집합하여 도서(book)에만 있는 도서 뷰(minus_book_view)를 생성하시오. 
create view minus_book_view as select a.bookid, a.bookkind, a.booktitle, a.bookcount, a.author, a.pubcom, a.pubdate from book a where not exists(select 1 from book2 b where a.bookid=b.bookid);

create view minus_book_view2 as select bookid, bookkind, booktitle, bookcount from book where bookid not in (select distinct bookid from book2);

create view minus_book_view3 as select a.bookid, a.bookkind, a.booktitle, a.bookcount from book a left join book2 b on a.bookid=b.bookid where b.bookid is null;
