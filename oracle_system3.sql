...
insert into member values('kkt', 'a1004', '김기태', '가산동 12', '010-1004-1004', '2023-09-12');
...
insert into sales values(sseq.nextval, '1', 'kkt', '2', null, default);
...
-- alter table member2 drop constraints idkey;                  primary key 삭제

-- 23) 도서2(book2) 테이블에 도서코드(bookind)를 기본키로 도서분류(bookkind)를 외래키로 추가하시오.
...
alter table book2 add constraint bfk2 foreign key (bookkind) references bookkind(kindcode);
...
select booktitle, bookprice, author from book where bookprice between 19000 and 30000;                      -- 동일
...
-- alter sequence sd_seq start with 6 increment by 1;(다른 DB에서는 가능)

-- 53) 상반기 판매순번 시퀀스(sd_seq)의 현재값이 조회될 수 있도록 하시오.
select c##sqltest.sd_seq.currval from dual;
...
select name from member where id not in (select id from sales);                 -- 구매한 적이 없는 회원 
...
minus select distinct a.booktitle, a.bookprice, a.author from book a right outer join sales b on a.bookid=b.bno;
...
create view tot_sales_view as (select * from sales UNION select * from sales2);                     -- 답
...
create view minus_book_view as (select bookid, bookkind, booktitle, bookprice from book MINUS select bookid, bookkind, booktitle, bookprice from book2);
